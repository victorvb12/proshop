...
// Slug → canonical value used in PRODUCTS.cat
const CANON = {
  men: "men's",
  ladies: "ladies",
  cosmetics: "cosmetics",
...
export const canonFromSlug = (slug) => CANON[slug] || null;
...
export const listByCanonCat = (slug) => {
  const canon = canonFromSlug(slug);
  return canon ? PRODUCTS.filter(p => p.cat === canon) : [];
};
